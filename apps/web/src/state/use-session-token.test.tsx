...
describe("useSessionToken", () => {
  ...
  it("persists manually entered tokens until they are explicitly cleared", () => {
    const { result } = renderHook(() => useSessionToken());

    act(() => result.current.setToken("  manual-token  "));

    expect(result.current.token).toBe("manual-token");
    expect(localStorage.getItem("vde-monitor-token")).toBe("manual-token");
